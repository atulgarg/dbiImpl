...
void q6 () {
	...
	Schema join_sch ("join_sch", outAtts, joinatt);
    Schema grp_sch("group_sch",1,&s_nationkey);

    Attribute prnt_sch[]= {DA, s_nationkey};
    Schema prnt_sch_s ("prnt_sch", 2,prnt_sch);
	GroupBy G;
		// _s (input pipe)
		Pipe _out (100);
		Function func;
	...
	Schema sum_sch ("sum_sch", 1, &DA);
	//int cnt = clear_pipe (_out, &sum_sch, true);
	int cnt = clear_pipe (_out, &prnt_sch_s, true);
