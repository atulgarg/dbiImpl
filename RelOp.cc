...
void* runSelectFile(void * sf)
{
    ...
    outPipe->ShutDown();
    cout<<"OutPipe Shutdown ho gayi select File main"<<endl;
}
...
void* runJoin(void *sf)
{
    ...
    {
        cout<<"sortOrder not 0 initialising Bigq"<<endl;
...
{
    //if(compEngine.Compare(&record, literal, cnf) == 0)
        outPipe->Insert(&record);
}
/**
...
void* RunGroupBy(void *gb)
{
    ...
    int* attsToKeep = new int[1+groupAtts->numAtts];
    attsToKeep[0] = 0;
    for(int i=1;i<=groupAtts->numAtts;i++)
        attsToKeep[i] = groupAtts->whichAtts[i-1];
    Record projectRecord;
    projectRecord.Copy(&prevRecord);
    projectRecord.Project(groupAtts->whichAtts, groupAtts->numAtts, groupAtts->numAtts);
    while(status != 0)
    {
        ...
        if(status!=0 && compEngine.Compare(&prevRecord, &record, groupAtts) != 0)
        {
            ...
            Record tempRecord;
            tempRecord.MergeRecords(&sumRecord, &projectRecord,1, groupAtts->numAtts, attsToKeep, 1+groupAtts->numAtts,1);
